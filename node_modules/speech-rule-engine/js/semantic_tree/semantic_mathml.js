...
export class SemanticMathml extends SemanticAbstractParser {
    ...
    sqrt_(_node, children) {
        ...
        return this.getFactory().makeBranchNode("sqrt", [SemanticProcessor.getInstance().row(semNodes)], []);
    }
    ...
    enclosed_(node, children) {
        const semNodes = this.parseList(SemanticUtil.purgeNodes(children));
        const newNode = this.getFactory().makeBranchNode("enclose", [SemanticProcessor.getInstance().row(semNodes)], []);
        newNode.role =
    ...
    leaf_(mml, children) {
        ...
        return this.getFactory().makeLeafNode(mml.textContent, SemanticProcessor.getInstance().font(mml.getAttribute('mathvariant')));
    }
}
