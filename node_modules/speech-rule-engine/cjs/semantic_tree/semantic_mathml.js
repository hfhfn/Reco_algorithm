...
class SemanticMathml extends semantic_parser_js_1.SemanticAbstractParser {
    ...
    parse(mml) {
        semantic_processor_js_1.default.getInstance().setNodeFactory(this.getFactory());
        ...
        if (['MATH', 'MROW', 'MPADDED', 'MSTYLE', 'SEMANTICS', 'MACTION'].indexOf(tag) !== -1) {
            return newNode;
        }
    ...
    rows_(node, children) {
        ...
        if (semantics && semantics.match('bspr_')) {
            return semantic_processor_js_1.default.proof(node, semantics, this.parseList.bind(this));
        }
        ...
        if (children.length === 1) {
            ...
            newNode = semantic_processor_js_1.default.getInstance().row(this.parseList(children));
        }
    ...
    fraction_(node, children) {
        ...
        const lower = children[1]
            ...
            : this.getFactory().makeEmptyNode();
        const sem = semantic_processor_js_1.default.getInstance().fractionLikeNode(upper, lower, node.getAttribute('linethickness'), node.getAttribute('bevelled') === 'true');
        return sem;
    }
    limits_(node, children) {
        return semantic_processor_js_1.default.getInstance().limitNode(DomUtil.tagName(node), this.parseList(children));
    }
    root_(node, children) {
        if (!children[1]) {
    ...
    sqrt_(_node, children) {
        ...
        return this.getFactory().makeBranchNode("sqrt", [semantic_processor_js_1.default.getInstance().row(semNodes)], []);
    }
    table_(node, children) {
        ...
        newNode.mathmlTree = node;
        return semantic_processor_js_1.default.tableToMultiline(newNode);
    }
    ...
    tableCell_(_node, children) {
        ...
        if (!semNodes.length) {
            ...
            childNodes = [semantic_processor_js_1.default.getInstance().row(semNodes)];
        }
    ...
    space_(node, children) {
        ...
        return semantic_processor_js_1.default.getInstance().text(newNode, DomUtil.tagName(node));
    }
    text_(node, children) {
    ...
    identifier_(node, children) {
        const newNode = this.leaf_(node, children);
        return semantic_processor_js_1.default.getInstance().identifierNode(newNode, semantic_processor_js_1.default.getInstance().font(node.getAttribute('mathvariant')), node.getAttribute('class'));
    }
    number_(node, children) {
        const newNode = this.leaf_(node, children);
        semantic_processor_js_1.default.number(newNode);
        return newNode;
    }
    operator_(node, children) {
        const newNode = this.leaf_(node, children);
        semantic_processor_js_1.default.getInstance().operatorNode(newNode);
        return newNode;
    }
    fenced_(node, children) {
        ...
        const newNode = semantic_processor_js_1.default.getInstance().mfenced(open, close, sepValue, semNodes);
        const nodes = semantic_processor_js_1.default.getInstance().tablesInRow([newNode]);
        return nodes[0];
    }
    enclosed_(node, children) {
        const semNodes = this.parseList(SemanticUtil.purgeNodes(children));
        const newNode = this.getFactory().makeBranchNode("enclose", [semantic_processor_js_1.default.getInstance().row(semNodes)], []);
        newNode.role =
    ...
    multiscripts_(_node, children) {
        ...
        if (!SemanticUtil.purgeNodes(lsup).length &&
            ...
            return semantic_processor_js_1.default.getInstance().pseudoTensor(base, this.parseList(rsub), this.parseList(rsup));
        }
        return semantic_processor_js_1.default.getInstance().tensor(base, this.parseList(lsub), this.parseList(lsup), this.parseList(rsub), this.parseList(rsup));
    }
    ...
    leaf_(mml, children) {
        ...
        return this.getFactory().makeLeafNode(mml.textContent, semantic_processor_js_1.default.getInstance().font(mml.getAttribute('mathvariant')));
    }
}
