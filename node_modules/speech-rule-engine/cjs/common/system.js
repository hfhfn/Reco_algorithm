...
function engineSetup() {
    const engineFeatures = ['mode'].concat(engine_js_1.default.STRING_FEATURES, engine_js_1.default.BINARY_FEATURES);
    const engine = engine_js_1.default.getInstance();
    const features = {};
    engineFeatures.forEach(function (x) {
        features[x] = engine[x];
    });
    features.json = system_external_js_1.default.jsonPath;
    features.xpath = system_external_js_1.default.WGXpath;
...
function processFile(processor, input, opt_output) {
    switch (engine_js_1.default.getInstance().mode) {
        ...
        default:
            throw new engine_js_1.SREError(`Can process files in ${engine_js_1.default.getInstance().mode} mode`);
    }
}
...
function processFileSync(processor, input, opt_output) {
    ...
    if (opt_output) {
        try {
            system_external_js_1.default.fs.writeFileSync(opt_output, result);
        }
        catch (err) {
...
function inputFileSync_(file) {
    let expr;
    try {
        expr = system_external_js_1.default.fs.readFileSync(file, { encoding: 'utf8' });
    }
    catch (err) {
...
function processFileAsync(processor, file, output) {
    return __awaiter(this, void 0, void 0, function* () {
        const expr = yield system_external_js_1.default.fs.promises.readFile(file, {
            encoding: 'utf8'
        });
        const result = ProcessorFactory.output(processor, expr);
        if (output) {
            try {
                system_external_js_1.default.fs.promises.writeFile(output, result);
            }
            catch (_err) {
...
if (system_external_js_1.default.documentSupported) {
