...
class SpeechRuleEngine {
    ...
    evaluateTree_(node) {
        ...
        for (let i = 0, component; (component = components[i]); i++) {
            ...
            if (attributes.engine) {
                saveEngine = engine_js_1.default.getInstance().dynamicCstr.getComponents();
                const features = grammar_js_1.Grammar.parseInput(attributes.engine);
                engine_js_1.default.getInstance().setDynamicCstr(features);
            }
            ...
            if (saveEngine) {
                engine_js_1.default.getInstance().setDynamicCstr(saveEngine);
            }
        }
        grammar_js_1.Grammar.getInstance().popState();
        return result;
    ...
    updateConstraint_() {
        const dynamic = engine_js_1.default.getInstance().dynamicCstr;
        const strict = engine_js_1.default.getInstance().strict;
    ...
    pickMostConstraint_(_dynamic, rules) {
        const comparator = engine_js_1.default.getInstance().comparator;
...
engine_js_1.default.nodeEvaluator = SpeechRuleEngine.getInstance().evaluateNode.bind(SpeechRuleEngine.getInstance());
