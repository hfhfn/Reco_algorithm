...
class MathSimpleStore {
    ...
    static testDynamicConstraints_(dynamic, rule) {
        if (engine_js_1.default.getInstance().strict) {
            return rule.cstr.equal(dynamic);
        }
        return engine_js_1.default.getInstance().comparator.match(rule.cstr);
    }
    ...
    defineRuleFromStrings(locale, modality, domain, style, content) {
        ...
        const parser = engine_js_1.default.getInstance().parsers[domain] ||
            engine_js_1.default.getInstance().defaultParser;
        const comp = engine_js_1.default.getInstance().comparators[domain];
        const cstr = `${locale}.${modality}.${domain}.${style}`;
        const dynamic = parser.parse(cstr);
        const comparator = comp ? comp() : engine_js_1.default.getInstance().comparator;
    ...
    lookupRule(_node, dynamic) {
        ...
        return rules.length
            ? rules.sort((r1, r2) => engine_js_1.default.getInstance().comparator.compare(r1.cstr, r2.cstr))[0]
            : null;
    }
}
