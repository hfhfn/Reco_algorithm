...
对于所有电影的平均评分 $$\mu$$ 是直接能计算出的，因此问题在于要测出每个用户的$$b_u$$值和每部电影的$$b_i$$的值。对于线性回归问题，我们可以利用平方差构建损失函数如下：<br />
...
$$
\begin{aligned}
\dfrac{\partial}{\partial b_u} J(\theta)&=\dfrac{\partial}{\partial b_u} f(b_u, b_i)
...
对损失函数求偏导：  
$$
\dfrac{\partial}{\partial b_u} f(b_u, b_i) =-2 \sum_{u,i\in R}(r_{ui}-\mu-b_u-b_i) + 2\lambda * b_u
$$
...
$$
b_u := \dfrac {\sum_{u,i\in R}(r_{ui}-\mu-b_i)}{\lambda_1 + |R(u)|}
$$
...
同理可得：  
$$
b_i := \dfrac {\sum_{u,i\in R}(r_{ui}-\mu-b_u)}{\lambda_2 + |R(i)|}
$$
